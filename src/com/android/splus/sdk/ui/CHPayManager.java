...
/**
 * @ClassName: CHPayManager
 * @author xiaoming.yuan
 * @date 2014-2-26 下午2:28:10
 */

public class CHPayManager implements IPayManager {
    private static final String TAG = "CHPayManager";

    public static final String SDK_VERSION = "1.0";

    private static CHPayManager mManager;
     ...
     */
    public static CHPayManager getInstance() {

        if (mManager == null) {
            synchronized (lock) {
                if (mManager == null) {
                    mManager = new CHPayManager();
                }
            }
        }
        return mManager;
    ...
    private synchronized void requestInit() {
        ...
        ActiveModel mActiveMode = new ActiveModel(mGameid, mPartner, mReferer, mac, imei, mWidth,
                mHeight, Phoneuitl.MODE, Phoneuitl.OS, Phoneuitl.OSVER, time, CommonUtil.getDebug(),
                sign);
    ...
    void onInitSuccess() {
        ...
        temp = mWaitTime - temp;
        temp = temp <= 0 ? 0 : temp;
        initHandler.sendEmptyMessageDelayed(0, temp);
    }
    ...
    void setUserData(UserModel mUserData) {
        this.mUserModel = mUserData;
        if (mUserData != null) {
            // 保存用户数据
            AppUtil.saveDatatoFile(mUserData);
