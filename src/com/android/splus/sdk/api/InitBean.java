...
public class InitBean {
    ...
    public static InitBean inflactBean(Activity activity, Properties prop, String appkey) {
        ...
        if (prop != null) {
         ...
         //   bean.setOrientation(Integer.parseInt(prop.getProperty("isLandScape") == null ? "1" : prop.getProperty("isLandScape")));
         //   bean.setFixed(Integer.parseInt(prop.getProperty("isFixed") == null ? "1" : prop.getProperty("isFixed"))); // 默认是1-定额
            bean.mAppkey = appkey;
        }
        return bean;
    }
    ...
    public void initSplus(final Activity activity, final InitCallBack initCallBack) {
        ...
        if (mUsesdk != PayManager.SPLUS) {
            init(activity, mAppkey, initCallBack, false, mOrientation == 1 ? Configuration.ORIENTATION_LANDSCAPE : Configuration.ORIENTATION_PORTRAIT);
        }
    }
    ...
    private synchronized void requestInit(Activity activity) {
        ...
        NetHttpUtil.getDataFromServerPOST(activity, new RequestModel(Constant.ACTIVE_URL,mActiveMode, new ActiveParser()), onActiveCallBack);
